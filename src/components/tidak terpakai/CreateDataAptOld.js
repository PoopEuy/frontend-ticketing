...
const CreateDataAptOld = () => {
  ...
  const handleCloseResult = (e) => {
    ...
    document.getElementById("nojs").value = "";
  };

  const handleClickOld = (e) => {
    console.log("Button Create OLD");
    getDataOld();
  };

  const getDataOld = async () => {
    console.log("Get Create OLD");
    try {
      const siteName = document.getElementById("siteName").value;
      const value_nojs = document.getElementById("nojs").value;
      const nojs = value_nojs.toUpperCase();
      ...
      const res = await instanceBECreateJson.post("aptOld", payload);
    ...
    <div>
      ...
      <div className="columns mt-5 is-centered">
        <div className="column is-half">
          ...
          <div className="input-group mb-3">
            <Row className="col sm row">
              <Form.Group as={Col} controlId="nojs">
                <Form.Label>Nojs</Form.Label>
                <Form.Control type="text" placeholder="Contoh : JS999" />
              </Form.Group>
            </Row>
          </div>
...
export default CreateDataAptOld;
